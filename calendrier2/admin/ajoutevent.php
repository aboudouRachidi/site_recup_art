...
<body>
	<?php
		...
		$titre=""; $description=""; $url_photo=""; $dateDebut = date("d/m/Y", time()); $dateFin = date("d/m/Y", time());
		...
		if(isset($_POST['envoi'])) {
			...
			$url_photo = nl2br(htmlentities(addslashes($_POST['url_photo'])));
			...
			if (preg_match($typeDate, $dateDebut) && preg_match($typeDate, $dateFin)) {
				...
				if(!empty($titre) && !empty($description) && !empty($url_photo)) {
					...
					$req = "INSERT INTO evenement VALUES ($identifiantCommun, '$titre', '$description','$url_photo')";
					mysql_query($req) or die(mysql_error());
    	...
    	<table id="tabAjoutEvent">
            ...
            <tr>
            	<td colspan="2">
       				...
       				<input type="text" name="url_photo" id="url_photo" size="30" value="<?php echo $url_photo?>" />
                </td>
            </tr>
            <tr>
